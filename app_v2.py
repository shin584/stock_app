...
def analyze_market_v2(market, date_str):
    ...
    strict_set, relaxed_set = get_consecutive_tickers_sets(market, valid_days)
    ...
    for ticker, row in df.iterrows():
        ...
        # 3. 3일 연속 순매수 필터 (일단 완화된 조건으로 통과시킴)
        if ticker not in relaxed_set:
            continue
...
st.markdown("""
**알고리즘 개요**
*   **1순위 (빈집털이)**: 프로그램 매도에도 불구하고 외국인과 기관(투신+연기금)이 받아내는 종목 (+100점)
*   **2순위 (정석 주도주)**: 외국인, 투신, 연기금 모두 매수 (+70점)
*   **3순위 (차선책)**: 3주체 중 2곳 이상 매수 (+40점)
*   **필터링**: 주가 급등(>15%) 제외, 금융투자 대량 매도 제외, **3일 연속 순매수(외국인/기관) 필수**
""")
